...
2. **Pick the safe write mode:**
   ...
   - **Section replace** (`action=replace` + `section_id`) is dangerous: a "section" block extends from the targeted element to the next real `#`/`##` heading, so targeting a bold pseudo-header paragraph (the `**Label**` style used in link-hub canvases) wipes everything below it to the end of the canvas. Only use it on true heading sections, and only when replacing that whole block is intended.
